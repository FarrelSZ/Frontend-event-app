...
const EventFilter = () => {
  const { control, setValue, dataCategory, isSuccessGetCategory } = useEventFilter();
  const {
  ...
  return (
    <div className="h-fit w-full rounded-xl lg:sticky lg:top-20  border p-4 lg:w-80">
          ...
          <Fragment>
            <Controller
              ...
              render={({ field: { onChange, ...field } }) => (
                ...
                >
                  {(category: ICategory) => (
                    <AutocompleteItem key={`${category._id}`}>{category.name}</AutocompleteItem>
                  )}
                </Autocomplete>
              )}
            />
