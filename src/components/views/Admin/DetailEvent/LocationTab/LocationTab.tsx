...
import useLocationTab from "./useLocation";
...
const LocationTab = (props: PropTypes) => {
  const { dataEvent, dataDefaultRegion, isPendingDefaultRegion, onUpdate, isPendingUpdate, isSuccessUpdate } = props;
  const {
            ...
            {!isPendingDefaultRegion ? (
              <Controller
                ...
                render={({ field: { onChange, ...field } }) => (
                  ...
                  >
                    {(regency: IRegency) => <AutocompleteItem key={`${regency.id}`}>{regency.name}</AutocompleteItem>}
                  </Autocomplete>
                )}
              />
            ) : (
