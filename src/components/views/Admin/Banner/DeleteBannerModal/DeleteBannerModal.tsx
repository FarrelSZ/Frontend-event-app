...
const DeleteBannerModal = (props: PropTypes) => {
  const { isOpen, onClose, onOpenChange, selectedId, setSelectedId, refetchBanners } = props;

  const { mutateDeleteBanner, isPendingMutateDeleteBanner, isSuccessMutateDeleteBanner } = useDeleteBannerModal();
    ...
    <Modal onOpenChange={onOpenChange} isOpen={isOpen} placement="center" scrollBehavior="inside">
      <ModalContent className="m-4">
        ...
        <ModalFooter>
          ...
          >
            {isPendingMutateDeleteBanner ? <Spinner size="sm" color="white" /> : "Delete Banner"}
          </Button>
        </ModalFooter>
      </ModalContent>
    </Modal>
